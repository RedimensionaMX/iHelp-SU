...
function get_registrosmes($seleccionados) {
  ...
  function get_datos_iniciales_recepcion_android() {
    $this->load->model('Tiposclasesequiposmodel');
    $registro = $this->get_campos();
    $registro['correo_electronico'] = '';
    $registro['telefono1'] = '';
    ...
    $registro['chlst_carcasa'] = 0;
    $registro['chlst_sensores'] = 0;
    $registro['chlst_rec_sensor_centro'] = 0;
    $registro['chlst_rec_sensor_humedad'] = 0;
    $registro['chlst_rec_falla_com_touch'] = 0;
    $registro['chlst_rec_falla_com_rueda'] = 0;
    $registro['chlst_rec_func_bot_home'] = 0;
    $registro['chlst_rec_func_bot_silencio'] = 0;
    $registro['caja_id'] = '';
    $tip1 = $this->Tiposclasesequiposmodel->get_tipos_dropdown();
    $tipant = array(""=>"Selecciona el tipo");
    $tip = array_merge($tipant,$tip1);
   ...
   function get_datos_iniciales_accesorio() {
    $this->load->model('Tiposclasesequiposmodel');

    $sucursal_id = $this->session->sucursal;
    $sucursal_id = explode(" ", $sucursal_id); //Devuelve la primer palabra del nombre de la sucursal
    //print_r($sucursal_id[0]);
    $registro = $this->get_campos();
    $registro['correo_electronico'] = '';
    $registro['telefono1'] = '';
    $registro['telefono2'] = '';
    ...
    $registro['colonia'] = '';
    $registro['cp'] = '';
    $registro['ciudad'] = $sucursal_id[0];
    $registro['estado'] = '';

    $registro['contrasenia'] = 'n/a';
    $registro['fecha_de_entrega'] = '';
    $registro['chlst_encendido'] = 0;
    $registro['chlst_lcd'] = 0;
    $registro['chlst_digitalizador'] = 0;
    ...
    $tipant = array(""=>"Selecciona el tipo");
    $tip = array_merge($tipant,$tip1);
    //print_r($tip['Accesorios']);

  
    $registro["num_orden"] = $this->Equiposmodel->get_numorden_no_incrementar();
    $registro["opciones_facturar"] = $this->Equiposmodel->opciones_facturar();
        //$registro["tipos"] = $tip['Accesorios'];
   // print_r(array_keys($tip)[1]);
        // $registro["tipos"] = array_keys($tip)[1];
   		$registro["tipos"] = $tip1;
        $registro["tipo"] = "";
       // print_r($tip);
   ...
   function get_datos_iniciales_equipo_venta() {
    $this->load->model('Tiposclasesequiposmodel');
...
/*       $str = $this->db->update_string('equipos',$p,array('id'=>$p['id']));
 ...
 function get_sqldesdebusqueda_fb($p,$usarlim = 1,$tipoorden = "",$lim) {
    ...
    if (count($p)>0) {
        ...
        if ((isset($p['busca_numorden'])) && ($p['busca_numorden']!="")) 
          $wh .= " and (num_orden='" . $p['busca_numorden'] . "')
           or (num_orden='" . substr ($sucursal_id,0,2) . $p['busca_numorden'] . "')";        
        //print_r($p['busca_numorden']);
              if ((isset($_POST['busca_numserie'])) && ($p['busca_numserie']!="")) 
          $wh .= " and num_serie='" . $p['busca_numserie'] . "'";
        ...
        if ((isset($p['busca_estatus'])) && ($p['busca_estatus']!="")) 
          ...
          $wh .= " and tipo='" . $p['busca_tipo'] . "'";  
              ...
              if ((isset($p['busca_nombre'])) && ($p['busca_nombre']!="")) {
          $arr = $this->db->query("select first 1400 id from clientes where upper(nombre) like upper('%" . 
                         $p['busca_nombre'] . "%')");
                $r = ($arr->result_array());
    ...
    $queryprincipal .= ' order by equipos.id ' . $tipoorden . ' ';

    
    if ($usarlim==1)   
      $queryprincipal = str_replace("select", "select first 35 skip " . $lim . " ",$queryprincipal);
  		//print_r($queryprincipal);
    return $queryprincipal;
   ...
   function get_detalle_equipo($id) {
      ...
      $equipo['devoluciones'] = $this->Movimientosmodel->get_devoluciones_de_equipo($id);
   ...
   function insertar_equipo($arreq){
    ...
    $arreq['ios_version'] = substr($arreq['ios_version'], 0,10);


    $this->Registroaccionesmodel->registrar("NUEVO","equipos",$_POST['num_orden']);

/*
$d = ($this->db->field_data('EQUIPOS'));
foreach ($d as $item) {
  $itemarray = get_object_vars($item);
   echo $itemarray['NAME'] . " " . $itemarray['MAX_LENGTH'] . " " . $itemarray['TYPE'] ."<br>";
}
print_r($d);
 die();*/
   ...
   function insertar_equipo2($arreq){
    $arreq = $this->filtra_post_guardar($arreq);
    $this->load->model("Registroaccionesmodel");
    $this->load->model("Movimientosmodel");
 ...
 die();*/
  ...
  function set_numero_remision_a_equipo($equipo_id) {
   ...
   function cancelar_orden($id) {
   	 $this->db->query("update equipos set situacion='X',fecha_cancelado=CURRENT_TIMESTAMP where id='" . $id . "'");
   	 return 1;
   }
...
/*
  ...
  function exportacion_datos($sucursal_id,$fecha1,$fecha2) {
 
    $qry = "select * from R_EXPORTACION_EQUIPOS('" . $sucursal_id . "','" . $fecha1 . "','" . $fecha2 . "') order by numeros_remisiones";
  ...
  function exportacion_equipos_movtos($sucursal_id,$fecha1,$fecha2) {
 
    $qry = "select * from  R_EXPORTACION_EQUIPOS_MOVTOS('" . $sucursal_id . "','" . $fecha1 . "','" . $fecha2 . "') order by numero_remision";
...
//print_r($a);
    ...
    function get_cierre_de_mes($anio,$mes) {
       $sucursal_id = $this->session->sucursal_id;      
    ...
    function existe_registro_numordenes($sucursal_id) {
    	$arrno =  $this->db->query("select * from numerosordenes where sucursal_id='" . $sucursal_id . "'");
     
	    $datano = $arrno->result_array();
      //print_r($datano);
	    if (count($datano)>0)
	       return TRUE;
	    else 
    ...
    function registro_numordenes($sucursal_id) {
      $arrno =  $this->db->query("select * from numerosordenes where sucursal_id='" . $sucursal_id . "'");
      $datano = $arrno->result_array();
      if (count($datano)>0)
         return $datano[0];
...
function get_correlaciones() {
  $arreq =  $this->db->query('select * from correlacion_android');
 $eq = ($arreq->result_array());
 return $eq;
 //return $eq[0]['clase'];
}  
...
function get_category(){
  $query = $this->db->query('select distinct tipo from correlacion_android');
  return $query;  
}

function get_sub_category($category_id){
  $query = $this->db->query("select distinct marca from correlacion_android where tipo = '".$category_id."'");
  //$query = $this->db->get_where('correlacion_android', array('TIPO' => $category_id));
  return $query;
}

function get_sub_category2($category_id,$tipo){
  $query = $this->db->query("select distinct modelo from correlacion_android where marca = '".$category_id."' and tipo = '" .$tipo."'");
  //$query = $this->db->get_where('correlacion_android', array('TIPO' => $category_id));
  return $query;
}
