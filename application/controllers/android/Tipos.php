...
class Tipos extends CI_Controller {
	...
	public function index(){
		$busca_nombre = "";
		$wh = "";
		$prueba = $this->uri->segment(7,'');
		if ($this->uri->segment(7,'')!=""){
			$_POST['clase'] = $this->uri->segment(7,'');
		}
		if (count($_POST)>0) {
			$wh = " where (1=1) ";
			...
			$this->load->model('android/Serviciosmodel');
			if($_POST['clase'] == ""){
				//print_r($_POST['clase']);
				$data['result'] = $this->Serviciosmodel->get_servicios_where_todos();
			} else{
				$data['result'] = $this->Serviciosmodel->get_servicios_where($_POST['clase']);
			}
            ...
            //print_r($data);
			$this->load->view('inicio/top1');
			$this->load->view('clases/indexandroid',$data);
			$this->load->view('inicio/bottom1');
		} else {
			$registro = array();
            $this->load->model('android/Clasesmodel');
			...
			$registro['clases'] = $this->Clasesmodel->get_clases_dropdown_todas();
			$this->load->view('inicio/top1');
			$this->load->view('servicios/seleccionarclaseandroid',$registro);
			$this->load->view('inicio/bottom1');
		}
	}
	...
	public function guardar() {
		...
		if ($accion=="i") {
			...
			redirect('/android/modelos/index/' . $_POST["marca"].'/'.$_POST["tipo"]);
		} else {
	...
	public function eliminar() {
		...
		$data = $this->Serviciosmodel->eliminar_modelo($this->uri->segment(4));
		//print_r($data[0]['tipo']);
		//print_r($data);
        //$this->Registroaccionesmodel->registrar("ELIMINADO","catservicios","ID: " . $this->uri->segment(3));
		redirect('/android/modelos/index/'.$data[0]['marca'].'/'.$data[0]['tipo']);
	}
